...
export const ParentLayout: React.FC = () => {
  ...
  const menuGroups: MenuGroup[] = [
    {
      title: 'Dashboard',
      items: [{ name: 'Dashboard', path: '/parent/dashboard', icon: LayoutDashboard }],
    },
    {
      title: 'Monitoring',
      items: [
        { name: 'Attendance', path: '/parent/attendance', icon: Calendar },
        { name: "Child's Performance", path: '/parent/performance', icon: TrendingUp },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Contact Teacher', path: '/parent/contact', icon: MessageSquare },
      ],
    },
    {
      title: 'Settings',
      items: [{ name: 'Profile', path: '/parent/profile', icon: User }],
    },
  ];
  ...
  const pageTitles: Record<string, string> = {
    ...
    '/parent/attendance': 'Attendance Monitor',
    '/parent/performance': "Child's Performance",
    '/parent/fees': 'Fee Status',
    '/parent/notifications': 'Notifications',
    '/parent/contact': 'Contact Teacher',
    '/parent/profile': 'My Profile',
  };

  const getPageTitle = () => pageTitles[location.pathname] || 'Parent Portal';
    ...
    <div className="min-h-screen max-w-full overflow-x-hidden flex text-neutral-900 dark:text-neutral-100 transition-colors duration-300 font-sans pb-6 pr-6">
      <aside className="fixed left-6 top-6 bottom-6 w-64 flex flex-col justify-between py-6 px-4 rounded-[32px] glass-bar z-40">
        <div className="flex flex-col w-full">
          <Link to="/parent/dashboard" className="flex items-center space-x-3 px-2 mb-6 hover:opacity-95 transition-all">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[12px] font-extrabold -mt-0.5" style={{ color: accentColor }}>Parent Portal</span>
            </div>
          </Link>
        ...
        <div className="pt-4 border-t border-neutral-200/50 dark:border-neutral-800/40 flex flex-col space-y-3 w-full">
          <div className="rounded-2xl p-3 border text-[10px] space-y-1.5" style={{ backgroundColor: `${accentColor}10`, borderColor: `${accentColor}25` }}>
            <div className="flex justify-between items-center">
              ...
              <span className="font-extrabold" style={{ color: accentColor }}>PARENT</span>
            </div>
            <div className="flex justify-between items-center">
          ...
          <Link to="/parent/contact" className="flex items-center space-x-2 text-[11px] font-semibold px-1 py-0.5 hover:underline" style={{ color: accentColor }}>
          ...
          <div className="flex items-center space-x-2.5 flex-shrink-0">
            ...
            <div className="relative" ref={profileMenuRef}>
              ...
              >
                ...
                />
                <span className="text-xs font-semibold pr-0.5 hidden md:block">{user?.name?.split(' ')[0] || 'Parent'}</span>
              ...
              {profileMenuOpen && (
                <div className="profile-dropdown absolute right-0 mt-2 w-52 border border-neutral-200/50 dark:border-neutral-800/40 rounded-2xl py-2 animate-fade-in z-50">
                  <div className="px-4 py-3 border-b border-neutral-100 dark:border-neutral-800/30">
                    ...
                    <p className="profile-dropdown-value text-sm font-extrabold">{user?.name || 'Parent'}</p>
                    <p className="text-[10px] mt-0.5 font-extrabold" style={{ color: accentColor }}>PARENT</p>
                  </div>
