...
export const HODLayout: React.FC = () => {
  ...
  const menuGroups: MenuGroup[] = [
    {
      title: 'Dashboard',
      items: [{ name: 'Dashboard', path: '/hod/dashboard', icon: LayoutDashboard }],
    },
    {
      title: 'Department',
      items: [
        { name: 'Department Students', path: '/hod/students', icon: Users },
        { name: 'Faculty Performance', path: '/hod/faculty', icon: GraduationCap },
        { name: 'Subject Management', path: '/hod/subjects', icon: BookOpen },
        { name: 'Timetable & Exams', path: '/hod/timetable', icon: Calendar },
        { name: 'Leave Management', path: '/hod/leave', icon: ClipboardList },
        { name: 'Grievances & Discipline', path: '/hod/grievances', icon: AlertTriangle },
      ],
    },
    {
      title: 'Analytics & Budget',
      items: [
        { name: 'Analytics', path: '/hod/analytics', icon: BarChart3 },
        { name: 'Reports', path: '/hod/reports', icon: FileText },
        { name: 'Budget & Resource', path: '/hod/budget', icon: DollarSign },
      ],
    },
    {
      title: 'Innovation & Placement',
      items: [
        { name: 'Research & Innovation', path: '/hod/research', icon: Award },
        { name: 'Placements & Placements', path: '/hod/partnerships', icon: Briefcase },
      ],
    },
    {
      title: 'Settings',
      items: [
        { name: 'Department Settings', path: '/hod/settings', icon: Settings },
  ...
  const subNavTabs = [
    ...
    { name: 'Analytics', path: '/hod/analytics' },
  ];

  const pageTitles: Record<string, string> = {
    '/hod/dashboard': 'HOD Dashboard',
    '/hod/students': 'Department Students',
    '/hod/faculty': 'Faculty Performance',
    '/hod/subjects': 'Subject Management',
    '/hod/timetable': 'Timetable & Exams',
    '/hod/leave': 'Leave Management',
    '/hod/grievances': 'Grievances & Discipline',
    '/hod/analytics': 'Department Analytics',
    '/hod/reports': 'Department Reports',
    '/hod/budget': 'Budget & Resource',
    '/hod/research': 'Research & Innovation',
    '/hod/partnerships': 'Placements & Placements',
    '/hod/settings': 'Department Settings',
    '/hod/profile': 'My Profile',
  };

  const getPageTitle = () => pageTitles[location.pathname] || 'HOD Portal';
    ...
    <div className="min-h-screen max-w-full overflow-x-hidden flex text-neutral-900 dark:text-neutral-100 transition-colors duration-300 font-sans pb-6 pr-6">
      <aside className="fixed left-6 top-6 bottom-6 w-64 flex flex-col justify-between py-6 px-4 rounded-[32px] glass-bar z-40">
        <div className="flex flex-col w-full">
          <Link to="/hod/dashboard" className="flex items-center space-x-3 px-2 mb-6 hover:opacity-95 transition-all">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[12px] font-extrabold -mt-0.5" style={{ color: accentColor }}>HOD Portal</span>
            </div>
          </Link>
        ...
        <div className="pt-4 border-t border-neutral-200/50 dark:border-neutral-800/40 flex flex-col space-y-3 w-full">
          <div className="rounded-2xl p-3 border text-[10px] space-y-1.5" style={{ backgroundColor: `${accentColor}10`, borderColor: `${accentColor}25` }}>
            <div className="flex justify-between items-center">
              ...
              <span className="font-extrabold" style={{ color: accentColor }}>HOD</span>
            </div>
            <div className="flex justify-between items-center">
          ...
          <Link to="/hod/analytics" className="flex items-center space-x-2 text-[11px] font-semibold px-1 py-0.5 hover:underline" style={{ color: accentColor }}>
          ...
          <div className="flex items-center space-x-2.5 flex-shrink-0">
            ...
            <div className="relative" ref={profileMenuRef}>
              ...
              >
                ...
                />
                <span className="text-xs font-semibold pr-0.5 hidden md:block">{user?.name?.split(' ')[0] || 'HOD'}</span>
              ...
              {profileMenuOpen && (
                <div className="profile-dropdown absolute right-0 mt-2 w-52 border border-neutral-200/50 dark:border-neutral-800/40 rounded-2xl py-2 animate-fade-in z-50">
                  <div className="px-4 py-3 border-b border-neutral-100 dark:border-neutral-800/30">
                    ...
                    <p className="profile-dropdown-value text-sm font-extrabold">{user?.name || 'HOD'}</p>
                    <p className="text-[10px] mt-0.5 font-extrabold" style={{ color: accentColor }}>HEAD OF DEPARTMENT</p>
                  </div>
